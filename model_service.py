...
asr_model = AutoModelForSpeechSeq2Seq.from_pretrained(
    MODEL_NAME, torch_dtype=torch_dtype, token=HF_TOKEN
).to(DEVICE)
processor = AutoProcessor.from_pretrained(MODEL_NAME, token=HF_TOKEN)
tokenizer = WhisperTokenizer.from_pretrained(
    MODEL_NAME, language=LANGUAGE, task="transcribe", token=HF_TOKEN
)
...
async def extract_address(audio: UploadFile = File(None), text: str = Form(None)):
    if audio:
        ...
        input_features = input_features.to(dtype=torch_dtype)  # <---- Исправление типа!
